...
# Created: Sun Dec 20 13:38:06 PST 2015
#
#/ usage: rundeck-adhoc:status  --execution <>  --url <$RUNDECK_URL> [ --username <$RUNDECK_USER>] [ --password <$RUNDECK_PASSWORD>] [ --apikey <$RUNDECK_APIKEY>] [ --detail <>] 
...
rerun_options_parse() {
    ...
    do
        OPT="$1"
        case "$OPT" in
            --execution) rerun_option_check $# $1; EXECUTION=$2 ; shift ;;
            ...
            --detail) rerun_option_check $# $1; DETAIL=$2 ; shift ;;
            # help option
            -|--*?)
                rerun_option_usage
                exit 2
    ...
    [[ -z "$EXECUTION" ]] && { echo >&2 "missing required option: --execution" ; return 2 ; }
